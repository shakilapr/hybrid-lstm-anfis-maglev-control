...
% 
% Description:
% This script implements section 3: data preprocessing.
% 
% Dependencies:
...
for i = 1:length(columnNames)
    ...
    title([varName, ' Before Clipping']);
    xlabel(varName);
    ylabel('Frequency');
    
    subplot(1,2,2);
    histogram(afterClipping, 50);
    title([varName, ' After Clipping']);
    xlabel(varName);
    ylabel('Frequency');
    
    figureFileName = ['Figure_', varName, '_Histograms_BeforeAfterClipping.png'];
    ...
    figureFileName = ['Figure_', varName, '_BoxPlots_BeforeAfterClipping.png'];
    saveas(gcf, fullfile(figureFolder, figureFileName));
    close(gcf);
end
...
figureFileName = 'Figure_CorrelationMatrixHeatmap_AfterPreprocessing.png';
saveas(gcf, fullfile(figureFolder, figureFileName));
close(gcf);
